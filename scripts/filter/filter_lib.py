# This file is derived from original library by Tom Verbeure
# https://github.com/tomverbeure/pdm/tree/master/modeling/cic_filters 
...
def reduce_bits(h, nr_bits):
    ...
    h_int = np.round(h / float_step).astype(int)
    h_round = h_int * float_step

    return h_round, h_int

def fir_calc_filter(Fs, Fpb, Fsb, Apb, Asb, N, verbose = True):
...
def half_band_calc_filter(Fs, Fpb, N, verbose = True):
